...
export const getCurrentUser = cache(async (): Promise<User | null> => {
  ...
  return queryOne<User>(
    'SELECT id, name, email, role, isActive, userScope, bureauScope, organizationName, phone, identityNumber, organizationLogoUrl, signatureUrl, createdAt, updatedAt FROM users WHERE id = ?',
    [session.userId]
  );
});
