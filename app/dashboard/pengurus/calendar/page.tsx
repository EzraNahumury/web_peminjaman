...
export default async function PengurusCalendar() {
  ...
  const blocks = await query<FacilityBlock & { facilityName: string | null }>(
    `SELECT b.*, f.name AS facilityName FROM facility_blocks b
     LEFT JOIN facilities f ON f.id = b.facilityId
     ORDER BY b.startDateTime ASC`
  );
  return (
    <div className="space-y-6">
      <PageHeader title="Kalender Fasilitas" subtitle="Jadwal fasilitas yang sudah dibooking, sedang diproses, atau diblokir admin." />
      <FacilityCalendar facilities={facilities} rows={rows} blocks={blocks} />
    </div>
  );
}
